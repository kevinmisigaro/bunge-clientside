...
export default function Index({ data }) {
  return (
    <>
      <PageSEO title={`Gender Champions`} description={`List of gender champions`} />
      <div className="mx-auto max-w-3xl divide-y divide-gray-200 px-4 dark:divide-gray-700 sm:px-6 xl:max-w-5xl xl:px-0">
        ...
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          {!data.champions.length && 'There are no gender champions currently.'}
          {data.champions.slice(0, MAX_DISPLAY).map((x, i) => {
            return (
